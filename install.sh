...
echo -n "Nonessential: "
if [ "${DO_NONESSENTIAL_INSTALL}" == "Y" ]; then echo "YES"; else echo "NO"; fi
echo -n "Unmount:      "
...
if [ "${DO_MOUNT}" == "Y" ]; then
	...
	umount "${OS_UEFI_PART}" || /bin/true
	umount "${OS_ROOT_PART}" || /bin/true
	swapoff "${OS_SWAP_PART}" || /bin/true
fi
...
if [ "${DO_EFI_ENTRY}" == "Y" ]; then
	echo "--- EFI ENTRY"
	efibootmgr --create --disk "/dev/${OS_HD_NAME}" --part 1 --label "Arch Linux" --loader /vmlinuz-linux --unicode "root=${OS_ROOT_PART} resume=${OS_SWAP_PART} rw initrd=\initramfs-linux.img"
else
...
if [ "${DO_ESSENTIAL_INSTALL}" == "Y" ] || [ "${DO_NONESSENTIAL_INSTALL}" == "Y" ]; then
	rm -rf /mnt/root/arch-framework || /bin/true
...
if [ "${DO_NONESSENTIAL_INSTALL}" == "Y" ]; then
	...
	arch-chroot /mnt /root/arch-framework/scripts/install-nonessential.sh
else
